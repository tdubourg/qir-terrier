 ...
 * under the License.
 *
 * The Original Code is MyIndexer.java.
 *
...
public class MyIndexer extends Indexer
{
	...
	protected class FieldTermProcessor implements TermPipeline
	{
		...
		public FieldTermProcessor()	{
			System.out.println("@@ Instanciating a new field term processor");
		}
	...
	protected MyIndexer(long a, long b, long c) {
		super(a,b,c);
		System.out.println("@@ Instanciating a new MyIndexer(a, b, c)");
	}

	/** 
	 * Constructs an instance of a MyIndexer, using the given path name
	 ...
	 */
	public MyIndexer(String path, String prefix) {
		super(path, prefix);
		System.out.println("@@ Instanciating a new MyIndexer(path, prefix)");
		//delay the execution of init() if we are a parent class
		if (this.getClass() == MyIndexer.class) 
			init();
	}
	 ...
	 * an instance of either MyIndexer.BasicTermProcessor, or 
	 * MyIndexer.FieldTermProcessor, depending on whether 
